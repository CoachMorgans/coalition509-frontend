/* ============================================================
 ...
 Version: 1.5.1 (Fix Stats + Paiement + Export CSV)
 ============================================================ */
...
function setupAdminFeatures() {
  ...
  if (ordersNav) ordersNav.style.display = isManagerUser ? '' : 'none';
...
function loadBotStats() {
  ...
  if (!container) {
    console.warn('[BOT STATS] Section #bot-stats-section introuvable');
    return;
  }
...
function loadOrders() {
  ...
  apiFetch('/api/v1/orders?per_page=100').then(function(res) {
    ...
    tbody.innerHTML = list.map(function(o) {
      ...
      if (o.payment_status === 'pending' || o.payment_status === 'unpaid') {
        payBtn = '<button onclick="openPaymentModal(' + o.id + ',' + (o.total_amount || 0) + ')" class="btn-icon" title="Payer">💳 Payer</button>';
      } else {
        payBtn = '<span style="color:#27ae60;font-weight:600;">✓ Paye</span>';
      }
...
function openPaymentModal(orderId, amount) {
  ...
  // Reset instructions et bouton confirmer
